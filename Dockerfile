...
RUN cd /heaptrack/profiler/profiler && \
    echo "./build.sh \$(ls -1 /heaptrack/coreclr-devel-*.armv7l.rpm | tail -n1) /rootfs" > build_docker.sh && /bin/bash build_docker.sh && \
    cp build/src/libprofiler.so /heaptrack-common/armel
